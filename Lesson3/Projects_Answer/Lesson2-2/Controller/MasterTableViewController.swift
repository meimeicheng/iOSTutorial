...
class MasterTableViewController: UITableViewController {
    var arr: [Restaurant] = []
    var dbBll : DataBll = DataBll()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        arr = dbBll.createRestaurant()
...
//    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.smallImageView.image = UIImage(named: arr[indexPath.row].image)
        cell.nameLabel.text = arr[indexPath.row].name
        cell.locationLabel.text = arr[indexPath.row].location
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail" {
            if let indexPath = tableView.indexPathForSelectedRow {
                ...
                destinationController.restaurant = arr[indexPath.row]
            }
        }
    }
